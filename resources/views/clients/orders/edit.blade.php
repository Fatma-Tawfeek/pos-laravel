...
<div class="breadcrumb-header justify-content-between">
    <div class="my-auto">
        <div class="d-flex">
            ...
            <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ @lang('orders.edit_title')</span>
        </div>
    </div>
</div>
<!-- breadcrumb -->
                                    ...
                                    <div class="col-md-6 col-lg-2">
                                        <div class="job-item">
                                            <div class="product-custom-card">
                                                <div 
                                                class="position-relative h-100 card add-product-btn {{ in_array($product->id, $order->products->pluck('id')->toArray()) ? 'border border-primary' : '' }}"
                                                {{ in_array($product->id, $order->products->pluck('id')->toArray()) ? 'style= pointer-events:none;' : '' }}
        ...
        <div class="card  box-shadow-0">
            ...
            <div class="card-body pt-0">
                ...
                <form action="{{ route('clients.orders.update', ['client' => $order->client, 'order' => $order]) }}" method="post">
                    ...
                    <h4>@lang('orders.total') : <span class="total-price">{{ number_format($order->total_price, 2) }}</span> {{ app('settings')['currency'] }} </h4>
                    <button class="btn btn-primary btn-block disabled" type="submit" id="add-order-form-btn">@lang('orders.edit_btn')</button>
                </form>
            </div>
        </div>
